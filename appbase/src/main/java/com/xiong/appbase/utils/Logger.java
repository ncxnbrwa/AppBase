...
public class Logger {
    private static String TAG = "AppBase";
    private static Context context;
    private static WebView webView;

    private static Boolean LOG_NEED_WRITE_TO_FILE = true;// 日志写入文件开关
    ...
    public static void init(Context appContext, WebView mWebView) {
        ...
        try {
            ...
            String pathName = Environment.getExternalStorageDirectory().getCanonicalPath() + "/"
                    + MyUtils.getAppProcessName(context);
    ...
    private static void log(String message, char level) {
        if (BuildConfig.mbDebugMode) {//日志文件总开关
